...
public interface PostRepository extends PagingAndSortingRepository<Post, Integer> {
  ...
  @Query(value = "SELECT p FROM Post p left join p.votes vote WHERE vote.value = 1 and p.isActive = 1 and p.moderationStatus = 'ACCEPTED' and p.time<current_timestamp() order by count(vote) desc")
